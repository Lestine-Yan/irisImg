...
func (_q *ApiKeyQuery) QueryImages() *ImageQuery {
...
func (_q *ApiKeyQuery) WithImages(opts ...func(*ImageQuery)) *ApiKeyQuery {
	query := (&ImageClient{config: _q.config}).Query()
...
func (_q *ApiKeyQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*ApiKey, error) {
	...
	if query := _q.withImages; query != nil {
		if err := _q.loadImages(ctx, query, nodes,
			func(n *ApiKey) { n.Edges.Images = []*Image{} },
			func(n *ApiKey, e *Image) { n.Edges.Images = append(n.Edges.Images, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *ApiKeyQuery) loadImages(ctx context.Context, query *ImageQuery, nodes []*ApiKey, init func(*ApiKey), assign func(*ApiKey, *Image)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(image.FieldKeyID)
	}
	query.Where(predicate.Image(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(apikey.ImagesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.KeyID
		if fk == nil {
			return fmt.Errorf(`foreign-key "key_id" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
